...
export const StakeForm = ({ mode, poolId, positionId }: StakeFormProps) => {
  ...
  const onSubmit = () => {
    if (mode === "new") {
      ...
      router.push(
        `/stake/pool/confirm-liquidity?positionId=${positionId}&isVested=${isVested}`,
      );
    }
  };
    ...
    <div className="z-10 flex flex-col items-center">
      ...
      >
        ...
        <div className="mt-[20px] w-[318px] md:hidden">
          {address ? (
            isCorrectChain ? (
              <Index onSlideComplete={onSubmit} disabled={!token0Amount}>
              ...
              >
                <div className="text-xs">Wrong Network</div>
              </Index>
            )
          ) : (
            <Index onSlideComplete={() => open()}>
              <div className="text-xs">Connect Wallet</div>
            </Index>
          )}
        </div>
