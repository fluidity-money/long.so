...
import { getFormattedStringFromTokenAmount } from '../util/converters'
...
type SeawaterResult<T extends string> = Awaited<
  ReturnType<typeof prepareWriteContract<typeof SeawaterABI, T, number>>
>['result']

// the strongly typed request value of the Seawater contract function `T`
type SeawaterRequest<T extends string> = Awaited<
  ReturnType<typeof prepareWriteContract<typeof SeawaterABI, T, number>>
>['request']

// the arguments for a given `SeawaterRequest<T>`
type SeawaterRequestArgs<T extends string> =
  'args' extends keyof SeawaterRequest<T> ? SeawaterRequest<T>['args'] : never
...
type UseSwap = ({ amountIn, minOut }: UseSwapProps) => {
   ...
   * Uses `token0` and `token1` from `useActiveTokenStore`, and `amountIn` and `minOut` from `UseSwapProps`. If one of the tokens is the fluid token, `swapPermit2` is called, otherwise `swap2ExactInPermit2` is called.
...
const useSwap: UseSwap = ({ amountIn, minOut }) => {
  const { token0, token1, decimals0, decimals1, ammAddress } =
    useActiveTokenStore()
  const { getPermit2Data } = usePermit2()

  const [prepareContractState, setPrepareContractState] = useState<
    PrepareContractState | undefined
  >()
  ...
  const result = useMemo(() => {
    ...
    const [, quoteAmountString] =
      error?.message.match(/reverted with the following reason:\n(.+)\n/) || []

    const result = [BigInt(amountIn), BigInt(quoteAmountString ?? 0)] as const
  ...
  const swap = async () => {
    try {
      ...
      if (!nonceHex || !sig || !encodedDeadline || !debouncedState) return
      ...
      const functionName =
        debouncedState?.functionName === 'quote'
          ? 'swapPermit2' // quote
      ...
      try {
        ...
        console.log('failed to write swap!', e)
      }
    } finally {
      setIsSwapping(false)
    }
