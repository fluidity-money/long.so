...
abigen!(LightweightERC20, "../out/LightweightERC20.sol/LightweightERC20.json");

fn must_address_from_env(env: &str) -> Address {
    match env::var(env) {
        Ok(addr) => Address::from_str(&addr).expect(&format!("Failed to parse an address from the env {}!", env)),
...
async fn main() {
    ...
    let mut test_state = TestState {
        ...
        swaps_addr, positions_addr, admin_addr, client,
    };

    let token = test_state.deploy_token_default(test_state.signer_addr).await;

    let amm = test_state.deploy_amm_default(token.address()).await;

    println!("address {:?}", amm.address());

    let token2 = test_state.deploy_token_default(test_state.signer_addr).await;
...
impl<M: Middleware, S: Signer> TestState<M, S> {
    async fn deploy_amm(&mut self, proxy_admin: Address, seawater_admin: Address, nft_manager: Address, fusdc_token: Address) -> SeawaterAMM<SignerMiddleware<M, S>> {
        SeawaterAMM::deploy(
            self.client.clone(),
            (
                proxy_admin,
    ...
    async fn deploy_amm_default(&mut self, fusdc_token: Address) -> SeawaterAMM<SignerMiddleware<M, S>> {
        self.deploy_amm(
            self.signer_addr,
            self.signer_addr,
            self.signer_addr,
    ...
    async fn deploy_token(&mut self, name: String, symbol: String, decimals: u8, total_supply: U256, recipient: Address) -> LightweightERC20<SignerMiddleware<M, S>> {
    ...
    async fn deploy_token_default(&mut self, recipient: Address) -> LightweightERC20<SignerMiddleware<M, S>> {
        self.deploy_token(
