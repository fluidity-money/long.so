...
func (r *amountResolver) ValueUsd(ctx context.Context, obj *model.Amount) (string, error) {
	...
	if obj.Token == r.C.FusdcAddr {
		return obj.ValueUnscaled.ScaleStr(6), nil
	}
...
func (r *queryResolver) GetPoolPositions(ctx context.Context, pool string, first *int, after *int) (positions model.SeawaterPositionsGlobal, err error) {
	...
	stmt := r.DB.Table("seawater_active_positions_6").
		Where("pool = ?", p).
		Limit(*first).
...
func (r *queryResolver) GetPosition(ctx context.Context, id int) (position *seawater.Position, err error) {
	...
	err = r.DB.Table("seawater_positions_5").
		Where("pos_id = ?", id).
		Scan(&position).
		Error
	return
...
// TotalFee is the resolver for the total_fee field.
func (r *seawaterPoolResolver) TotalFee(ctx context.Context, obj *seawater.Pool) (model.TotalFee, error) {
	if obj == nil {
		return model.TotalFee{}, fmt.Errorf("no pool obj")
	}
	var totalFee model.TotalFee
	err := r.DB.Table("seawater_pool_fees_total_1").
		Where("pool = ?", obj.Token).
		Scan(&totalFee).
		Error
	if err != nil {
...
func (r *seawaterPoolResolver) VolumeOverTime(ctx context.Context, obj *seawater.Pool) (vol model.VolumeOverTime, err error) {
	...
	err = r.DB.Table("seawater_pool_swap_volume_daily_2").
	...
	err = r.DB.Table("seawater_pool_swap_volume_monthly_2").
...
func (r *seawaterPoolResolver) PositionsForUser(ctx context.Context, obj *seawater.Pool, wallet string, first *int, after *int) (positions model.SeawaterPositionsUser, err error) {
	...
	err = r.DB.Table("seawater_active_positions_6").
...
func (r *seawaterPoolResolver) Swaps(ctx context.Context, obj *seawater.Pool, first *int, after *int, filter *string) (swaps model.SeawaterSwaps, err error) {
	...
	// DB.RAW doesn't support chaining
	err = r.DB.Raw(
		"SELECT * FROM seawater_swaps_pool_5(?, ?, ?, ?, ?, ?)",
		r.C.FusdcAddr,
		r.C.FusdcDecimals,
		obj.Token,
		time.Unix(int64(*after), 0),
